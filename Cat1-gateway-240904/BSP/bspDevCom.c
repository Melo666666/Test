...
s16 HoldReg_Dev[DEV_NUM_MAX][e_DevHoldReg_Number]={0};			// 面板参数;
...
//	if(E180_UartPara.Rcved)
	{
		...
		if(pRcv[0] == tempUnit.slaveAddr &&  rcvLen>5 &&
			pRcv[0]>=DEV_SLAVE_MIN_ADDR && pRcv[0]<=DEV_SLAVE_MAX_ADDR)				//地址正确
		{
			calCRC = usMBCRC16(pRcv, rcvLen-2);						//计算所接收数据的CRC
			recCRC = pRcv[rcvLen-2] | (pRcv[rcvLen-1]<<8);			//接收到的CRC(低字节在前，高字节在后)
